...
import { db, functions, storage } from "../services/firebaseConfig";
...
// Signed URL (Cloud Function) ή fallback σε getDownloadURL
async function getSignedOrDownloadURL(storagePath, tries = 5, delayMs = 240) {
  ...
  for (let i = 0; i < tries; i++) {
    try {
      ...
      const shouldRetryCF =
        transientMsg.includes("not-found") || transientMsg.includes("no-url");
    ...
    try {
      ...
      // race after upload -> retry
    }

    if (i < tries - 1) {
      await new Promise((r) =>
        setTimeout(r, delayMs * Math.pow(1.7, i))
      );
    }
  }
...
async function handleIncomingShare({
  ...
  if (!chatId || !myEmail || !item || !uid || !canWrite) return;
  ...
  if (Array.isArray(item.items) && item.items.length) {
    for (const sub of item.items) {
      await handleIncomingShare({ chatId, myEmail, item: sub, uid, canWrite });
    }
    return;
  }
...
const MessageBubble = React.memo(
  function MessageBubble({
    ...
    const imgSize = Math.min(420, Math.floor(screenW * 0.8));
        ...
        >
          {!!msg.text && (
            <ParsedText
              ...
              parse={[
                { type: "url", style: styles.linkText, onPress: onPressLink },
              ]}
              selectable
            >
              {msg.text}
...
/* ========================= Main Screen ========================= */
export default function ChatScreen({ route, user, navigation }) {
  const {
    chatId,
    programTitle,
    subscriptionId,
  ...
  const [canWrite, setCanWrite] = useState(initialIsActive);
  ...
  const isReadyForChat = !!chatId && !!uid && !!myEmail;
  ...
  useEffect(() => {
    if (!user?.uid || !subscriptionId) {
      setCanWrite(initialIsActive !== false);
      return;
    }
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        ...
        setCanWrite(isSubscriptionActive(data));
      },
      (err) => {
    ...
    return () => unsub();
  }, [user?.uid, subscriptionId, initialIsActive]);
  ...
  }, [chatId, isReadyForChat]);

  // Load older
  const loadOlder = useCallback(async () => {
    const now = Date.now();
    if (now - loadOlderDebounce.current < 400) return;
  ...
  }, [chatId, messages, hasMore, loadingOlder, isReadyForChat]);
  ...
  useEffect(() => {
    if (!isReadyForChat || isReadOnly) return;

    const ShareMenu = getShareMenu();
    if (!ShareMenu) return;

    const run = (item) => {
      if (!item || !isReadyForChat || isReadOnly) return;
    ...
    try {
      if (typeof ShareMenu.getInitialShare === "function") {
        ...
        ShareMenu
          ...
          .then((text) => {
            if (text && !isReadOnly) {
              run({ mimeType: "text/plain", data: String(text) });
            }
          })
          .catch(() => {});
      }
  ...
  }, [chatId, myEmail, uid, isReadyForChat, isReadOnly]);
  ...
  const sendMessage = async () => {
    if (isReadOnly) {
      Alert.alert(
        "Read only chat",
        "Το πρόγραμμα έχει λήξει. Μπορείς μόνο να δεις το ιστορικό."
      );
      return;
    }
  ...
  const pickImage = async () => {
    if (isReadOnly) {
      return Alert.alert(
        "Read only chat",
        "Το πρόγραμμα έχει λήξει. Δεν μπορείς να στείλεις εικόνες."
      );
    }

    if (!isReadyForChat) {
      return Alert.alert("Error", "Not authenticated or no chat selected.");
    }
  ...
  const pickVideo = async () => {
    if (isReadOnly) {
      return Alert.alert(
        "Read only chat",
        "Το πρόγραμμα έχει λήξει. Δεν μπορείς να στείλεις βίντεο."
      );
    }
  ...
  const saveCurrentImage = useCallback(async () => {
    ...
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Permission needed",
          "Please allow media library access to save the image."
        );
        setSaving(false);
        return;
      }
    ...
    ({ item }) =>
      item.type === "day" ? (
        ...
        <MessageBubble
          ...
          openUrl={(u) =>
            Linking.openURL(u).catch(() => Alert.alert("Cannot open", u))
          }
  ...
  return (
    ...
    >
      ...
      {isReadOnly && (
        <View style={styles.readOnlyBanner}>
          <Text style={styles.readOnlyBannerText}>
            Το πρόγραμμα έχει λήξει. Μπορείς να δεις το ιστορικό του chat, αλλά
            δεν μπορείς να στείλεις μηνύματα ή αρχεία.
          </Text>
        </View>
      )}
      ...
      <View style={styles.inputBar}>
        <Pressable
          onPress={pickImage}
          disabled={isReadOnly || !isReadyForChat}
          style={({ pressed }) => [
            styles.circleBtn,
            (isReadOnly || !isReadyForChat) && styles.disabledBtn,
            pressed && { opacity: 0.7 },
          ]}
        >
        ...
        <View style={[styles.inputWrap, isReadOnly && styles.inputWrapDisabled]}>
          <TextInput
            style={[styles.input, isReadOnly && styles.inputDisabled]}
            placeholder={
              isReadOnly
                ? "Το πρόγραμμα έχει λήξει. Το chat είναι μόνο για ανάγνωση."
                : "Message…"
            }
            value={input}
            ...
            autoCorrect={false}
            multiline
            editable={isReadyForChat && !isReadOnly}
            onFocus={() =>
              setTimeout(
                () =>
        ...
        <Pressable
          onPress={sendMessage}
          disabled={!isReadyForChat || isReadOnly}
          style={({ pressed }) => [
            styles.sendBtn,
            (!isReadyForChat || isReadOnly) && styles.disabledSendBtn,
      ...
      >
        <View style={styles.viewerOverlay}>
          ...
          <View style={styles.viewerInner}>
            ...
            <View style={styles.viewerButtonsRow}>
              <Pressable style={styles.viewerButton} onPress={saveCurrentImage}>
                {saving ? (
              ...
              <Pressable
                style={[styles.viewerButton, { backgroundColor: "#6b7280" }]}
                onPress={() => setViewerVisible(false)}
              >
